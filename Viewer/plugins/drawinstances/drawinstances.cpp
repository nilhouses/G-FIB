...
bool DrawInstances::drawObject(int i)
{
  ...
  if (numInstances == 1) {
    // Pintar una sola instància a l'origen (ID = 0)
    g.glDrawArrays(GL_TRIANGLES, 0, numIndices[i]);
  }
  else {
    // Pintar múltiples instàncies: una graella 3x3
    g.glDrawArraysInstanced(GL_TRIANGLES, 0, numIndices[i], numInstances);
  }// TODO - End
...
void DrawInstances::preFrame() 
{
  ...
  program->setUniformValue("oneInstance",(numInstances == 1));
}
